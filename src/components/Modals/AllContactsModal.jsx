...
const AllContactsModal = ({ handleModal }) => {
  ...
  const fetchAllContacts = async () => {
    try {
      ...
      if (page === 1) {
        setAllContacts((pre) => [...response?.data?.results]);
      } else {
        setAllContacts((pre) => [...pre, ...response?.data?.results]);
      }
    } catch (error) {
        ...
        <div className="modal-dialog modal-dialog-scrollable" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h4 className="modal-title mx-auto fw-bold"> All Contacts</h4>
            </div>
            <div className="modal-body" onScroll={handleScroll} ref={modalRef}>
              {showEven
